...
#! sbatch --array=0- ./hpc_train_eval_good_dtu.sh

CONF_DIR="./configs/hpc_confs"
BATCH_NAME="dtu_new_exp"
# BATCH_NAME="dtu_ls"
...
CONFS=("less_trunc") 
# CONFS=("less_sparse") 
# CONFS=("less_trunc_less_sparse") 
# CONFS=("less_trunc_2") 
# CONFS=("less_trunc_black") 
# CONFS=("less_trunc_lowscale") 
# CONFS=("smooth") 
CONFS=("less_trunc_norm_59") 
for CONF in ${CONFS[@]}
do
    CONF_NAME=${CONF}.yaml
    ...
    # python opt.py -t $CKPT_DIR $DATA_DIR -c $CONFIG \
    #     --load_pretrain_density_sh "/rds/project/rds-qxpdOeYWi78/plenoxels/opt/ckpt/tuning/dtu_scan${SCAN}/nerf/syn_black" --near_clip $NEAR_CLIP
    # python opt.py -t $CKPT_DIR $DATA_DIR -c $CONFIG \
    #     --load_pretrain_density_sh "/rds/project/rds-qxpdOeYWi78/plenoxels/opt/ckpt/tuning/dtu_scan${SCAN}/nerf/syn_lowscale" --near_clip $NEAR_CLIP
    ...
    do
        ...
        IN_PATH=$OUT_PATH/"$(basename $ckpt_name .npz)"/vis_$(printf "%03d" $SCAN)_d2s.ply
        ...
        xvfb-run -a python /rds/project/rds-qxpdOeYWi78/plenoxels/opt/scripts/vis_pt_mesh_dtu.py \
            --input_path $IN_PATH --out_dir $OUT_DIR --scan $SCAN
    ...
    CKPT_PATHS=$CKPT_DIR/ckpt*.npz
    OUT_PATH=$CKPT_DIR/ckpt_eval_surf_mlv
    mkdir -p $OUT_PATH
    for ckpt_path in ${CKPT_PATHS[@]}
    do
        ckpt_name=$(basename $ckpt_path)
        ...
        python exract_points_no_cam.py $CKPT_DIR/$ckpt_name --downsample_density 0.2 \
            --out_path $PT_PATH --intersect_th -1 --n_sample 5 --data_path $DATA_DIR
