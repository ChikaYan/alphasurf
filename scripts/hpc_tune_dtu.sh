...
#! sbatch --array=0- ./hpc_tune_dtu.sh

CONF_DIR="./configs/auto_tune"
BATCH_NAME="dtu_less_trunc_norm"
...
do
    ...
    python eval_dtu.py --input_path $PT_PATH --scan $SCAN --dataset_dir "$DTU_DIR" --out_dir $OUT_PATH/"$(basename $ckpt_name .npz)" \
        --log_tune_hparam_config_path "$CONF_DIR/$BATCH_NAME/config.json" --hparam_save_name "hparam_surf_close_mlv"
done
