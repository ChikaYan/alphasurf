...
do
    ...
    # ours no fs masked surf
    IN_PATHS+=("/rds/project/rds-qxpdOeYWi78/plenoxels/opt/ckpt/tuning/$SCENE/good_trunc/solid_no_fs/ckpt_eval_surf_masked/ckpt/vis_d2s.ply")
    # ours no fs single surf
    IN_PATHS+=("/rds/project/rds-qxpdOeYWi78/plenoxels/opt/ckpt/tuning/$SCENE/good_trunc/solid_no_fs/ckpt_eval_surf_single/ckpt/vis_d2s.ply")

    # # ours semi-t no fs mask surf
    # IN_PATHS+=("/rds/project/rds-qxpdOeYWi78/plenoxels/opt/ckpt/tuning/$SCENE/good_trunc/trans_no_fs_low_sparse_2/ckpt_eval_surf_masked/ckpt/vis_d2s.ply")
    # # ours semi-t no fs single surf
    # IN_PATHS+=("/rds/project/rds-qxpdOeYWi78/plenoxels/opt/ckpt/tuning/$SCENE/good_trunc/trans_no_fs_low_sparse_2/ckpt_eval_surf_single/ckpt/vis_d2s.ply")
    ...
    # OUT_DIR="$(dirname $IN_PATH)/imgs_gt_pt"
    # echo Visualizing: $IN_PATH

    # xvfb-run -a python /rds/project/rds-qxpdOeYWi78/plenoxels/opt/scripts/vis_pt_mesh.py \
    #     --input_path $IN_PATH --out_dir $OUT_DIR --no_color --extra_ele -30

    OUT_DIR="$(dirname $IN_PATH)/imgs_gt_pt_crop"
    echo Visualizing: $IN_PATH

    xvfb-run -a python /rds/project/rds-qxpdOeYWi78/plenoxels/opt/scripts/vis_pt_mesh.py \
        --input_path $IN_PATH --out_dir $OUT_DIR --no_color --mask_crop
