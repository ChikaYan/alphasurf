...
#! sbatch --array=0- ./hpc_train_eval_batch_nerf_dtu.sh

CONF_DIR="./configs/hpc_confs"
BATCH_NAME="nerf"

CONF_NAME=syn.yaml
CONF_NAME=syn_lowscale.yaml

# SCANS=(63 24 37 69)
# SCANS=(37 40 55 65 69 97 106 110 114 118 122) #~10
...
python opt.py -t $CKPT_DIR $DATA_DIR -c $CONFIG --near_clip $NEAR_CLIP

python render_imgs_circle.py $CKPT_DIR $DATA_DIR --renderer_backend $BACKEND --near_clip $NEAR_CLIP --radius $RADIUS

python render_imgs_circle.py $CKPT_DIR $DATA_DIR --renderer_backend $BACKEND --render_depth --depth_thresh 0.1 --near_clip $NEAR_CLIP --radius $RADIUS
...
do
    ...
    python exract_points_no_cam.py $CKPT_DIR/$ckpt_name --downsample_density 0.2 \
        --out_path $PT_PATH --intersect_th $T --n_sample 5 --data_path $DATA_DIR
...
do
    ...
    do
        ...
        python exract_points_depth.py $CKPT_DIR/$ckpt_name $DATA_DIR --traj_type "spiral" --num_views 100 \
            ...
            --out_path $PT_PATH --weight_thresh $W --radius $RADIUS
...
#! sbatch --array=4- ./hpc_train_eval_batch_nerf_dtu.sh
